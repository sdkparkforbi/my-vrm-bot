const SCHOOL_API_BASE = process.env.SCHOOL_API_BASE || 'https://aiforalab.com/liveavatar-api/api.php'
const ALLOWED_ACTIONS = new Set([
  ...
  'usage_summary',
  // ─── 개인 봇 지식(RAG) + 페르소나 ───
  'rag_list',      // owner_id → 청크 전체 (공개 읽기)
  'rag_search',    // owner_id, query, k → 관련 top-k
  'rag_save',      // owner_id, chunks[] (+auth)
  'rag_delete',    // owner_id, chunk_id (+auth)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 개인 봇 격리: Vercel env BOT_ID → X-Bot-Id 헤더로 학교 서버 PHP에 전달.
    // PHP가 로그/설문/RAG 데이터의 소유자(owner_id) 식별에 사용.
    const BOT_ID = process.env.BOT_ID
    if (BOT_ID) {
      fwdHeaders['X-Bot-Id'] = String(BOT_ID)
    }
